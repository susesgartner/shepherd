...
func CreateProvisioningAKSHostedCluster(client *rancher.Client) (*management.Cluster, error) {
	...
	config.LoadConfig(cloudcredentials.AzureCredentialConfigurationFileKey, &cloudCredentialConfig.AzureCredentialConfig)
	...
	clusterResp, err := aks.CreateAKSHostedCluster(client, clusterName, cloudCredential.ID, aks.ClusterConfig{}, false, false, false, false, nil)
	if err != nil {
		return nil, err
	}
...
func CreateProvisioningEKSHostedCluster(client *rancher.Client) (*management.Cluster, error) {
	...
	clusterResp, err := eks.CreateEKSHostedCluster(client, clusterName, cloudCredential.ID, eks.ClusterConfig{}, false, false, false, false, nil)
	if err != nil {
		return nil, err
	}
...
func CreateProvisioningGKEHostedCluster(client *rancher.Client) (*management.Cluster, error) {
	...
	clusterResp, err := gke.CreateGKEHostedCluster(client, clusterName, cloudCredential.Namespace+":"+cloudCredential.Name, gke.ClusterConfig{}, false, false, false, false, nil)
	if err != nil {
		return nil, err
	}
